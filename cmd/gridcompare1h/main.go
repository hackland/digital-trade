...
func main() {
	...
	combos := []combo{
		...
		{"macd45+ema35+mfi20", []interface{}{mod("macd", 0.45), mod("ema_cross", 0.35), mod("mfi", 0.2)}},
	}
	buyThresholds := []float64{0.15, 0.20, 0.25, 0.30}
	sellThresholds := []float64{-0.15, -0.20, -0.25, -0.30}
	confirmBarsOpts := []int{1, 2}
	cooldownBarsOpts := []int{6, 12, 24, 48}
	minHoldBarsOpts := []int{6, 12, 24, 48}
	atrStopMultOpts := []float64{3.0}
